    ...
    def choose_best_split(self, X_subset, y_subset, prev_predictions, sample_weights):
        ...
        for i in range(len(X_subset[0])):
            ...
            for thread in thresholds:
                ...
                if gain > best_gain and gain>=self.gamma_regularizer and cover_L>self.cover and cover_R>self.cover:
                    threshold = thread
                    feature_index = i
                    best_gain = gain
        if best_gain<0:
...
class BoostForest():
    def __init__(self,  max_depth=np.inf, min_samples_split=2, 
                 random_state=None, 
                 depth=3, num_trees=10, cover=1,
                 lr=0.3, decay_rate=1, decay_interval=None, max_features=None,
